module Suspension
    module Diff
        ...
        # returns absolute start/stop values -> [[10,22]] or  [[10,23]]. Deletions are relative to original text. Additions are relative to text with deletions already applied.
        def self.convert_diff_match_patch (diff_match_patch_list, segment_type)
            dmp = diff_match_patch_list
            raise "diff_match_patch_list must contain an array of number/text pairs. #{dmp.inspect} " if dmp.any? { |p| !([-1,0,1].include?(p[0]) && p[1].is_a?(String)) }
    ...
    class AbsoluteSuspendedTokens
        def with_deletions deletions
            raise "Array of ordered begin/end pairs expected" if deletions.any? {|a| a.length != 2} || deletions.flatten.reduce(0) { |result, e| e >= result ? e : false } === false 
        ...
        def with_additions additions, affinity = :left
            ...
            raise "Array of ordered begin/end pairs expected" if additions.any? {|a| a.length != 2} || additions.flatten.reduce(0){ |result, e| e >= result ? e : false } === false 
            AbsoluteSuspendedTokens.new(self.map do |token| 
                token = token.dup
